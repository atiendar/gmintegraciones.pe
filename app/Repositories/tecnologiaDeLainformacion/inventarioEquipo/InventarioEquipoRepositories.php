...
class InventarioEquipoRepositories implements InventarioEquipoInterface {
      ...
      DB::transaction(function()use($request, $id_inventario) {  // Ejecuta una transacción para encapsulan todas las consultas y se ejecuten solo si no surgió algún error
        ...
        if($inventario->isDirty()) {
          ...
          ActividadRegistrada::dispatch(
            ...
            array('Empresa', 'Responsable', 'Número de Serie', 'Marca', 'Modelo', 'Creación Inventario Equipo', 'Actualización Inventario Equipo', 'Descripción del Equipo', 'Observaciones'), //Nombre de lsos Inputs del formulario 
            $inventario, //Request
            array('emp', 'resp', 'num_ser', 'mar', 'mod', 'created_at_inv_equ', 'updated_at_inv_equ', 'des_del_equ', 'obs')
          );
    ...
    public function eliminarCacheAllinventarioPlunk(){
      cache::pull('allInventariosPlunk');
    }
