...
class ArmadoRepositories implements ArmadoInterface {
  ...
  public function armadoAsignadoFindOrFailById($id_armado, $clon) {
    ...
    $armado = Armado::asignado(Auth::user()->registros_tab_acces, Auth::user()->email_registro)->where('clon', $clon)->with('productos', 'imagenes')->findOrFail($id_armado);
    return $armado;
  }
  ...
  // Calcula el nuevo alto, ancho, largo precio cliente y peso del armado en dado caso que aya cambiado el valor del producto
  public function calcularNuevosValoresDelArmado($producto, $nuev_alto, $nuev_ancho, $nuev_largo, $nuev_prec_clien, $nuev_pes) {
    $hastaC = count($producto->armados()->withTrashed()->get()) -1;
    $producto_armados = $producto->armados()->withTrashed()->get();
    for($contador2 = 0; $contador2 <= $hastaC; $contador2++) {
      // Calcula el nuevo alto del armado
      if($producto->getOriginal('alto') != $nuev_alto) {
        $producto_armados[$contador2]->alto -= $producto->getOriginal('alto') * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->alto += $nuev_alto * $producto_armados[$contador2]->pivot->cant;
      }

      // Calcula el nuevo ancho del armado
      if($producto->getOriginal('ancho') != $nuev_ancho) {
        $producto_armados[$contador2]->ancho -= $producto->getOriginal('ancho') * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->ancho += $nuev_ancho * $producto_armados[$contador2]->pivot->cant;
      }

      // Calcula el nuevo largo del armado
      if($producto->getOriginal('largo') != $nuev_largo) {
        $producto_armados[$contador2]->largo -= $producto->getOriginal('largo') * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->largo += $nuev_largo * $producto_armados[$contador2]->pivot->cant;
      }

      // Calcula el nuevo precio cliente del armado
      if($producto->getOriginal('prec_clien') != $nuev_prec_clien) {
        $producto_armados[$contador2]->prec_origin -= $producto->getOriginal('prec_clien') * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->prec_origin += $nuev_prec_clien * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->prec_redond = $this->calculoRepo->redondearUnidadA9DelPrecio($producto_armados[$contador2]->prec_origin);
      }

      // Calcula el nuevo peso del armado
      if($producto->getOriginal('pes') != $nuev_pes) {
        $producto_armados[$contador2]->pes -= $producto->getOriginal('pes') * $producto_armados[$contador2]->pivot->cant;
        $producto_armados[$contador2]->pes += $nuev_pes * $producto_armados[$contador2]->pivot->cant;
      }

      if($producto_armados[$contador2]->isDirty()) {
        // Dispara el evento registrado en App\Providers\EventServiceProvider.php
        ActividadRegistrada::dispatch(
          'Armados', // Módulo
          'armado.show', // Nombre de la ruta
          $this->serviceCrypt->encrypt($producto_armados[$contador2]->id), // Id del registro debe ir encriptado
          $producto_armados[$contador2]->id, // Id del registro a mostrar, este valor no debe sobrepasar los 100 caracteres
          array('Alto', 'Ancho', 'Largo', 'Precio original', 'Precio redondeado', 'Peso'), // Nombre de los inputs del formulario
          $producto_armados[$contador2], // Request
          array('alto', 'ancho', 'largo', 'prec_origin', 'prec_redond','pes') // Nombre de los campos en la BD
        ); 
        $producto_armados[$contador2]->updated_at_arm = Auth::user()->email_registro;
      }
      $producto_armados[$contador2]->save();
    }
  }
}