...
class SustitutoArmadoController extends Controller {
  ...
  public function create($id_producto) {
    ...
    $produc_original  = $this->productoRepo->getproductoFindOrFailById($this->serviceCrypt->encrypt($producto->id_producto), ['sustitutos']);
  ...
  public function destroy($id_sustituto) {
    try { DB::beginTransaction();
      ...
      $producto_original1         = $this->productoRepo->getproductoFindOrFailById($this->serviceCrypt->encrypt($sustituto->producto->id_producto), []);
      ...
      $producto_original2         = $this->productoRepo->getproductoFindOrFailById($this->serviceCrypt->encrypt($sustituto->id_producto), []);
