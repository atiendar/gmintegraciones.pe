...
<div class="card">
  ...
  <div class="card-body"> 
      ...
      {!! Form::model(Request::all(), ['route' => 'sistema.plantilla.index', 'method' => 'GET']) !!}
        <div style="float: right;">
          <div class="input-group input-group-sm" style="width: 25em;">
            {!! Form::select('opcion_buscador', config('opcionesSelect.select_plantilla_index'), null, ['class' => 'form-control float-right']) !!}
    ...
    <div class="pt-2">
      <div style="float: right;">
        {!! $plantillas->appends(Request::all())->links() !!}  
      </div>
      {{ __('Mostrando desde') . ' '. $plantillas->firstItem() . ' ' . __('hasta') . ' '. $plantillas->lastItem() . ' ' . __('de') . ' '. $plantillas->total() . ' ' . __('registros') }}.
    </div>
  </div>
</div>
@endsection