...
<td width="1rem" title="Eliminar: {{ $direccion->est }}">
  <form method="post" action="{{ route('cotizacion.armado.direccion.destroy', Crypt::encrypt($direccion->id)) }}" id="cotizacionArmadoDireccionDestroy{{ $direccion->id }}">
    @method('DELETE')@csrf
    {!! Form::button('<i class="far fa-trash-alt"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-sm', 'id' => "btnsub$direccion->id", 'onclick' => "return check('btnsub$direccion->id', 'cotizacionArmadoDireccionDestroy$direccion->id', '¡Alerta!', '¿Estás seguro quieres eliminar el registro, $direccion->cant ($direccion->est) ?', 'info', 'Continuar', 'Cancelar', 'false');"]) !!}
  </form>
</td>