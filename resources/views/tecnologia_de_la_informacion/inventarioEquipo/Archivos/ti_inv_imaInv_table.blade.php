        ...
        @foreach ($archivos as $archivo)
          <td title="{{ __('Fecha de registro') }}: {{ $archivo->created_at }}, {{ __('Registrado por') }}: {{ $archivo->created_at }}, {{ __('Nombre') }}: {{ $archivo->arc_nom }}">
            <div class="card" style="width: 12rem;">
              ...
              @endif
                <img src="{{ Storage::url($archivo->arc_rut.$archivo->arc_nom) }}" class="card-img-top" alt="{{ $archivo->arc_nom }}">
                  <div class="card-body">
                    <p class="card-text text-muted text-sm">
                      <a href="{{ Storage::url($archivo->arc_rut.$archivo->arc_nom) }}" download>{{ $archivo->arc_nom }}</a>
                    </p>
                  </div>
            </div>
          </td>
