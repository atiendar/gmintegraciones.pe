...
<td width="1rem" title="Eliminar: {{ $pedido->nom }}">
  @can('venta.pedidoActivo.destroy')
    <form method="post" action="{{ route('venta.pedidoActivo.destroy', Crypt::encrypt($pedido->id)) }}" id="ventaPedidoActivoDestroy{{ $pedido->id }}">
      @method('DELETE')@csrf
      {!! Form::button('<i class="far fa-trash-alt"></i>', ['type' => 'submit', 'class' => 'btn btn-danger btn-sm', 'id' => "btnsub$pedido->id", 'onclick' => "return check('btnsub$pedido->id', 'ventaPedidoActivoDestroy$pedido->id', '¡Alerta!', '¿Estás seguro quieres eliminar el registro, $pedido->serie.$pedido->id ($pedido->usuario->email_registro) ?', 'info', 'Continuar', 'Cancelar', 'false');"]) !!}
    </form>
  @endcan
</td>