<div class="row">
  ...
  <div class="form-group col-sm btn-sm">
    <label for="los_armados_son_de_stock">{{ __('¿Los armados son de stock?') }} *</label>
    <div class="input-group">
      <div class="input-group-prepend">
        <span class="input-group-text"><i class="fas fa-list"></i></span>
      </div>
      {!! Form::select('los_armados_son_de_stock', ['Si' => 'Si', 'No' => 'No'], null, ['class' => 'form-control select2' . ($errors->has('los_armados_son_de_stock') ? ' is-invalid' : ''), 'placeholder' => __('')]) !!}    
    </div>
    <span class="text-danger">{{ $errors->first('los_armados_son_de_stock') }}</span>
  </div>
</div>
<div id="manual">
  <div class="row">
...
<div class="row">
  ...
  <div class="form-group col-sm btn-sm">
    <button type="submit" id="btnsubmit" class="btn btn-info w-100 p-2"><i class="fas fa-check-circle text-dark"></i> {{ __('Registrar armado') }}</button>
  </div>
</div>
