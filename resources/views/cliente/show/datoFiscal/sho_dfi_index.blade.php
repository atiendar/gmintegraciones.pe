...
<div class="card card-info card-outline card-tabs position-relative bg-white">
  <div class="card-body">
    <div class="pb-1">
      ...
      <div style="float: right;">
        <div class="input-group input-group-sm" style="width: 25em;">
          {!! Form::select('opcion_buscador', config('opcionesSelect.select_datos_fiscales_index'), null, ['class' => 'form-control float-right']) !!}
    ...
    <div class="pt-2">
      <div style="float: right;">
        {!! $datos_fiscales->appends(Request::all())->links() !!}  
      </div>
      {{ __('Mostrando desde') . ' '. $datos_fiscales->firstItem() . ' ' . __('hasta') . ' '. $datos_fiscales->lastItem() . ' ' . __('de') . ' '. $datos_fiscales->total() . ' ' . __('registros') }}.
    </div>  
  </div>
</div>
@endsection