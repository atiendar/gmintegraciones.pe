...
<div class="card card-info card-outline card-tabs position-relative bg-white">
  <div class="card-body">
    <div class="pb-1">
      ...
      <div style="float: right;">
        <div class="input-group input-group-sm" style="width: 25em;">
          {!! Form::select('opcion_buscador', config('opcionesSelect.select_cliente_qys_index'), null, ['class' => 'form-control float-right']) !!}
    ...
    <div class="pt-2">
      <div style="float: right;">
        {!! $quejas_y_sugerencias->appends(Request::all())->links() !!}  
      </div>
      {{ __('Mostrando desde') . ' '. $quejas_y_sugerencias->firstItem() . ' ' . __('hasta') . ' '. $quejas_y_sugerencias->lastItem() . ' ' . __('de') . ' '. $quejas_y_sugerencias->total() . ' ' . __('registros') }}.
    </div>
  </div>
</div>
@endsection