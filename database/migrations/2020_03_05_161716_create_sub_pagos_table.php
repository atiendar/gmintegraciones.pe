...
class CreateSubPagosTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('sub_pagos', function (Blueprint $table) {
            $table->engine ='InnoDB';
            ...
            $table->unsignedBigInteger('pago_id')->comment('Foreign Key del código de factura');
            $table->foreign('pago_id')->references('id')->on('pagos')->onUpdate('restrict')->onDelete('cascade');
            $table->string('created_at_sub_pag',75)->nullable()->comment('Correo del usuario que realizo el registro');
            $table->string('updated_at_sub_pag',75)->nullable()->comment('Correo del usuario que realizo la última modificación');
            $table->timestamps();
            $table->softDeletes();
        });
    }
